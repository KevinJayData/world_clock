...
def digital_clock():
    label0.config(text='Sydney Australia \n' + datetime.now(pytz.timezone('Australia/Sydney')).strftime("%b %d %Y %I:%M:%S %p"))
    label1.config(text='Tel Aviv \n' + datetime.now(pytz.timezone('Israel')).strftime("%b %d %Y %I:%M:%S %p"))
    label2.config(text='New York \n' + datetime.now(pytz.timezone('US/Eastern')).strftime("%b %d %Y %I:%M:%S %p")) # 'US/Eastern'
    label3.config(text='Chicago \n' + datetime.now(pytz.timezone('US/Central')).strftime("%b %d %Y %I:%M:%S %p")) # 'US/Central'
    label4.config(text='Denver \n' + datetime.now(pytz.timezone('US/Mountain')).strftime("%b %d %Y %I:%M:%S %p")) # 'US/Mountain'
